        ...
        PerformanceMetrics? metrics = null)
{
    ...
    /// <exception cref="SkillissueException">If an event handler is not registered.</exception>
    /// <returns>A <see cref="Task"/> representing the asynchronous operation.</returns>
    public async Task Dispatch(Event e)
    {
        if (metrics is not null)
            await metrics.MeasureAsync(e.GetType().Name, () => Handle(e));
        else
            await Handle(e);

        if (++_eventCount % 1000 == 0)
        {
            metrics?.Print(e.Time.ToString());
        }
    }
    ...
    private async Task Handle(Event e)
    {
        switch (e)
        {
            case ArriveAtStation ev:
                ...
                eventSubscriber?.OnArrivalAtStation(ev);
                break;
            case EndCharging ev:
                stationService.HandleEndCharging(ev);
                eventSubscriber?.OnChargingEnd(ev);
                break;
