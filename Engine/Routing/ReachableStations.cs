...
public class ReachableStations
{
    ...
    /// </summary>
    /// <param name="segments">The direct route to the EV's destination.</param>
    ...
    public static List<ushort> FindReachableStations(Segments segments, EV ev, Dictionary<ushort, Station> stations, List<ushort> nearbyStations, double radius)
    {
        ...
        return [.. nearbyStations.Where(id =>
            {
                var dist = GeoMath.DistancesThroughPath(segments, stations[id].Position, radius);
