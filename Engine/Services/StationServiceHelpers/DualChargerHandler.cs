    ...
    : IChargerHandler
{
    ...
    private void ScheduleEndEvents(IntegrationResult? result)
    {
        ...
        {
            var token = scheduler.ScheduleEvent(new EndCharging(state.SessionA.EVId, charger.Id, finishA));
            state.SessionA = state.SessionA with { CancellationToken = token };
        }

        if (state.SessionB is not null && result?.FinishTimeB is { } finishB)
        {
            var token = scheduler.ScheduleEvent(new EndCharging(state.SessionB.EVId, charger.Id, finishB));
