...
public class EVStoreTests
{
    ...
    public void TryAllocate()
    {
        ...
        var success = evStore.TryAllocate(500, (_, ref ev) => ev = new EV(new Battery(1, 1, 1, Socket.CCS2), new Preferences(1.5f), journey), evIndexes);
        Assert.True(success);
        ...
        var realloc = evStore.TryAllocate(1, (_, ref ev) => ev = new EV(new Battery(1, 1, 1, Socket.CCS2), new Preferences(1.5f), journey));
        Assert.True(realloc);
        Assert.Equal(0, evStore.AvailableCapacity());
    }
    ...
    public void SetGet()
    {
        ...
        var success = evStore.TryAllocate(1, (_, ref ev) => ev = new EV(new Battery(1, 1, 1, Socket.CCS2), new Preferences(1.5f), journey));
        ...
        var newEv = new EV(new Battery(1, 1, 1, Socket.CCS2), new Preferences(2.0f), journey);
